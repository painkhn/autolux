        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    Редактировать марку {{ $brand->title }}
                </h3>
            ...
            <!-- Modal body -->
            <div class="p-4 md:p-5 space-y-4 -mt-4">
                <form method="POST" action="{{ route('brand.update', ['id' => $brand->id]) }}" class="space-y-4" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div>
                        <x-label for="title" value="Название" />
                        <x-input id="title" type="text" name="title" value="{{ $brand->title }}" placeholder="Введите название авто" />
                    </div>
                    <div>
